...
1. 调用 create-inspection-task（创建一次性巡检任务）：
...
请检查以下可能原因：
  • AccessKey 权限是否包含 rdsai 相关 API（rdsai:create-inspection-task、rdsai:get-inspection-report）
  • 是否存在可用的 RDS 实例
  • 网络连接是否正常
...
```json
{
  "agents": {
    "rds-inspection-cron": {
      ...
      "message": "执行 RDS 数据库定期巡检：调用 create-inspection-task（instance-ids=all），等待后调用 get-inspection-report 获取报告，将报告摘要发送到通知渠道。",
